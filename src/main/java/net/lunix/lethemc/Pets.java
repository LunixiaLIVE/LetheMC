...
import net.lunix.lethemc.mixin.FoxAccessor;
...
public final class Pets {
    ...
    public static boolean isWarded(Entity entity) {
        if (!(entity instanceof OwnableEntity ownable)) {
            // Neither is ownable; both are warded only while the whole animal is doomed.
            if (entity instanceof Fox fox) return isFoxWarded(fox);
            if (entity instanceof Villager villager) return isVillagerWarded(villager);
            return false;
        }
    ...
    private static boolean isFoxWarded(Fox fox) {
        ...
        Map<UUID, String> stamps = stamped.lethemc$trustStamps();
        if (stamps.isEmpty()) return false;

        int s0 = slotState(fox, FoxAccessor.lethemc$trusted0(), stamps);
        int s1 = slotState(fox, FoxAccessor.lethemc$trusted1(), stamps);
    ...
    /**
     * True while every customer of a villager is in Purgatory with their remains intact.
     *
     * <p>Without this the trading hall has the same hole a chest donkey had, worked from the
     * other direction. A doomed villager is spared the moment anyone living trades with it --
     * that is the whole point of the rule -- so during the grace period a friend can walk the
     * hall, buy one thing from each villager, and the entire hall survives a death that was
     * meant to take it. The owner is offline and cannot object.
     *
     * <p>So while its last customer is in Purgatory the villager cannot be traded with at all.
     * A villager any living player still deals with is never warded, because it was never at
     * risk: the hall stays open to the people it already belonged to.
     *
     * <p><b>Asks the ledger, not the stamps</b>, for the same reason the fox ward does. The
     * customer's incarnation has not rotated yet -- that happens when the remains are destroyed
     * at the end of the grace period -- so nothing is stale during the very window this covers.
     */
    private static boolean isVillagerWarded(Villager villager) {
        if (!Config.get().wipeVillagers) return false;
        Object gossips = villager.getGossips();
        if (!(gossips instanceof DirectlyKnown known) || !(gossips instanceof LifeStamped stamped)) return false;

        Set<UUID> direct = known.lethemc$direct();
        if (direct.isEmpty()) return false;

        Map<UUID, String> stamps = stamped.lethemc$stamps();
        boolean anyDoomed = false;
        for (UUID customer : direct) {
            // Dealings we cannot date are never acted on, so there is nothing to protect.
            if (stamps.get(customer) == null) return false;

            Ledger.Entry e = Ledger.get(customer);
            // No entry means alive and well; PARDONED means resurrected and on the way back.
            // Either way somebody living trades here and the villager is in no danger.
    ...
    public static void stamp(Entity animal, LivingEntity owner) {
        ...
        stamped.lethemc$setIncarnation(Incarnations.of(owner.getUUID()));
    }
    ...
    public static void check(Entity entity) {
        ...
        String stamp = stamped.lethemc$getIncarnation();
        ...
        LetheMC.LOGGER.info("Removing {} at {} -- its owner is living a different life",
    ...
    private static void checkFox(Fox fox) {
        ...
        EntityDataAccessor<Optional<EntityReference<LivingEntity>>> slot0 = FoxAccessor.lethemc$trusted0();
        EntityDataAccessor<Optional<EntityReference<LivingEntity>>> slot1 = FoxAccessor.lethemc$trusted1();
        ...
        LetheMC.LOGGER.info("Removing fox at {} -- no living player trusts it",
    ...
    private static void forget(Fox fox,
        ...
        LetheMC.LOGGER.info("Fox at {} forgot a player whose life has ended",
                fox.blockPosition().toShortString());
    }
    ...
    private static void checkVillager(Villager villager) {
        ...
        LetheMC.LOGGER.info("Removing villager at {} -- everyone who traded here is gone",
