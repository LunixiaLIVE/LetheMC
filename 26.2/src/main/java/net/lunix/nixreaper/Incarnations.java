...
public final class Incarnations {
    ...
    private static final Path PATH = FabricLoader.getInstance().getConfigDir()
            .resolve("nixreaper").resolve("incarnations.json");
    ...
    public static String rotate(UUID player) {
        ...
        current.put(player.toString(), id);
        save();
        NixReaper.LOGGER.info("Rotated incarnation for {} -- anything tamed in the old life is now loose", player);
        return id;
    }
    ...
    public static void load() {
        ...
        try (Reader r = Files.newBufferedReader(PATH)) {
            ...
            NixReaper.LOGGER.info("Loaded {} incarnation record(s)", current.size());
        } catch (Exception e) {
            NixReaper.LOGGER.error("Failed to load incarnations; pets will not be released this session", e);
        }
    }
    ...
    public static void save() {
        try {
            ...
            NixReaper.LOGGER.error("Failed to save incarnations", e);
        }
    }
